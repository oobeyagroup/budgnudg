...
class TestResolveTransactionAISuggestions(TestCase):
    ...
    def setUp(self):
        ...
        self.subcategory1 = Category.objects.create(
            name="Restaurants", type="expense", parent=self.category1
        )
    ...
    def test_ai_suggestions_equal_current_values_same_colors_disabled_buttons(self):
        ...
        with patch(
            "transactions.categorization.categorize_transaction_with_reasoning"
        ) as mock_categorize:
            ...
            url = reverse(
                "transactions:categorize_transaction", kwargs={"pk": transaction.pk}
            )
            ...
            if current_badge and suggestion_badge:
                ...
                self.assertEqual(
                    current_badge.get_text().strip(),
                    suggestion_badge.get_text().strip(),
                )
                ...
                if train_button:
                    ...
                    self.assertTrue(
                        "disabled" in train_button.get("class", [])
                        or train_button.get("disabled") == "disabled"
                        or "btn-outline-secondary" in train_button.get("class", [])
                    )
                ...
                if apply_button:
                    self.assertTrue(
                        "disabled" in apply_button.get("class", [])
                        or apply_button.get("disabled") == "disabled"
                        or "btn-outline-secondary" in apply_button.get("class", [])
                    )
    ...
    def test_payoree_suggestion_from_description_matching(self):
        ...
        url = reverse(
            "transactions:categorize_transaction", kwargs={"pk": transaction_exact.pk}
        )
        response = self.client.get(url)
        ...
        if suggested_payoree_name:
            ...
            payoree_exists = Payoree.objects.filter(
        ...
        if suggested_payoree_partial:
            ...
            payoree_exists = Payoree.objects.filter(
                name=suggested_payoree_partial
            ).exists()
            self.assertTrue(payoree_exists)
    ...
    def test_payoree_suggestion_integration_in_template(self):
        ...
        if ai_suggestions:
            ...
            self.assertGreaterEqual(
                len(rows), 2, "Should have header row and at least one data row"
            )
