...
def _create_chk_account():
    # helper to create the FinancialAccount used in tests
    from ingest.models import FinancialAccount

    # Use get_or_create so repeated calls in the same test session don't
    # attempt to insert a duplicate unique 'name' row.
    obj, created = FinancialAccount.objects.get_or_create(
        name="CHK-1001",
        defaults={"description": "Test checking account", "column_map": {}},
    )
    # Ensure column_map is present for older fixtures
    if not obj.column_map:
        obj.column_map = {}
        obj.save(update_fields=["column_map"])
    return obj
...
class TestTransactionModel:
    ...
    def test_transaction_creation_with_minimal_fields(self):
        ...
        assert transaction.bank_account.name == "CHK-1001"
    ...
    def test_transaction_creation_with_all_fields(self):
        ...
        subcategory = Category.objects.create(
            name="Restaurants", type="expense", parent=category
        )
    ...
    def test_transaction_str_truncation(self):
        ...
        long_description = (
            "This is a very long transaction description that should be truncated"
        )
        transaction = Transaction.objects.create(
            source="test.csv",
            bank_account=_create_chk_account(),
...
class TestTransactionCategorization:
    ...
    def test_clean_method_validates_subcategory_parent(self):
        ...
        restaurant_subcat = Category.objects.create(
            name="Restaurants", parent=food_category
        )
...
class TestTransactionDisplayMethods:
    ...
    def test_effective_category_display_with_error(self):
        ...
        assert (
            transaction.effective_category_display()
            == "ERROR: AI_NO_SUBCATEGORY_SUGGESTION"
        )
