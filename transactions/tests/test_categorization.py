...
class TestSafeLookups:
    ...
    def test_safe_category_lookup_multiple_categories_prefers_top_level(self):
        ...
        subcategory = Category.objects.create(
            name="Food", type="expense", parent=parent_cat
        )
    ...
    def test_safe_category_lookup_multiple_categories_no_top_level(self):
        ...
        subcat1 = Category.objects.create(
            name="Restaurant", type="expense", parent=parent1
        )
        subcat2 = Category.objects.create(
            name="Restaurant", type="expense", parent=parent2
        )
...
class TestCategorization:
    ...
    def test_categorize_transaction_with_reasoning_includes_explanation(self):
        ...
        category, subcategory, reasoning = categorize_transaction_with_reasoning(
            "STARBUCKS #1234", 5.75
        )
...
class TestLearningSystem:
    ...
    def test_top_learned_subcat_with_data(self):
        ...
        LearnedSubcat.objects.create(
            key="starbucks", subcategory=subcategory, count=5, last_seen=dt.date.today()
        )
    ...
    def test_top_learned_subcat_multiple_entries_returns_highest(self):
        ...
        LearnedSubcat.objects.create(
            key="starbucks", subcategory=coffee_subcat, count=8
        )
        LearnedSubcat.objects.create(
            key="starbucks", subcategory=restaurant_subcat, count=3
        )
    ...
    def test_top_learned_payoree_with_data(self):
        ...
        LearnedPayoree.objects.create(
            key="starbucks", payoree=payoree, count=7, last_seen=dt.date.today()
        )
    ...
    def test_suggest_payoree_uses_learned_data(self):
        ...
        LearnedPayoree.objects.create(
            key="starbucks", payoree=payoree, count=5, last_seen=dt.date.today()
        )
...
class TestConfidenceCalculation:
    ...
    def test_confidence_calculation_high_learned_data(self):
        ...
        confidence = calculate_suggestion_confidence(
            "STARBUCKS #1234", suggested_category="Food", suggested_subcategory="Coffee"
        )

        assert (
            confidence["overall_confidence"] == 90.0
        )  # Actual confidence for 15 confirmations
        assert confidence["source"] == "learned"
        assert confidence["learning_count"] == 15
        assert (
            confidence["subcategory_confidence"] == 95.0
        )  # Raw subcategory confidence
    ...
    def test_confidence_calculation_medium_learned_data(self):
        ...
        assert (
            confidence["overall_confidence"] == 80.0
        )  # Actual confidence for 5 confirmations
        assert confidence["source"] == "learned"
        assert confidence["learning_count"] == 5
        assert (
            confidence["subcategory_confidence"] == 85.0
        )  # Raw subcategory confidence
    ...
    def test_confidence_calculation_low_learned_data(self):
        ...
        LearnedSubcat.objects.create(
            key="starbucks", subcategory=subcategory, count=1, last_seen=dt.date.today()
        )
        ...
        assert (
            confidence["overall_confidence"] == 52.5
        )  # Actual confidence for 1 confirmation
        assert confidence["source"] == "learned"
        assert confidence["learning_count"] == 1
        assert (
            confidence["subcategory_confidence"] == 55.0
        )  # Raw subcategory confidence
...
class TestKeywordRules:
    ...
    def test_check_keyword_rules_matching_rule(self):
        ...
        # check_keyword_rules returns (category, subcategory, reasoning, payoree)
        category, subcategory, reasoning, _payoree = result
        assert category == "Food"
        assert subcategory == "Coffee"
        assert "keyword" in reasoning.lower()
...
class TestIntegrationScenarios:
    ...
    def test_learned_data_overrides_default_categorization(self):
        ...
        coffee_subcat = Category.objects.create(name="Coffee", parent=food_category)
        meeting_subcat = Category.objects.create(
            name="Client Meetings", parent=business_category
        )
