...
class CleanFullTransactionReportView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ).filter(date__gte=start_date).order_by("-date", "description")
        ...
        category_type_data = defaultdict(lambda: defaultdict(lambda: {
            'name': '',
            'category_type': '',
            'category_obj': None,
            'total_amount': Decimal('0'),
        ...
        for category_type in type_order:
            if category_type in category_type_data:
                ...
                type_total_amount = sum(cat_data['total_amount'] for cat_data in type_categories.values())
                type_total_count = sum(cat_data['total_count'] for cat_data in type_categories.values())
