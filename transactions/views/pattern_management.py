...
def pattern_management(request):
    ...
    subcat_patterns = (
        LearnedSubcat.objects.values("key")
        .annotate(
    ...
    payoree_patterns = (
        LearnedPayoree.objects.values("key")
        .annotate(
            total_count=Sum("count"),
...
def pattern_detail(request, pattern_key):
    ...
    subcats = LearnedSubcat.objects.filter(key=pattern_key).select_related(
        "subcategory__parent"
    )
    ...
    # Test current extraction (removed - using payoree relationships instead)
    current_extraction = None
    if sample_transactions.exists():
        # No longer extracting merchants - using payoree relationships
        pass
    ...
    context = {
        ...
        "extraction_matches": False,  # No longer relevant
    }
...
def merge_patterns(request):
    ...
    try:
        ...
        if old_key == new_key:
            return JsonResponse(
                {"success": False, "error": "Cannot merge pattern with itself"}
            )
...
def rename_pattern(request):
    ...
    try:
        ...
        if old_key == new_key:
            return JsonResponse(
                {"success": False, "error": "Old and new keys are the same"}
            )

        # Check if new key already exists
        if (
            LearnedSubcat.objects.filter(key=new_key).exists()
            or LearnedPayoree.objects.filter(key=new_key).exists()
        ):
            return JsonResponse(
                {
                    "success": False,
...
def test_extraction(request):
    ...
    if request.method == "POST":
        try:
            ...
            extracted = description.upper().strip()  # Use description directly as key

            # Check for existing patterns
            subcat_matches = list(
                LearnedSubcat.objects.filter(key=extracted).values(
                    "subcategory__name", "subcategory__parent__name", "count"
                )
            )
            payoree_matches = list(
                LearnedPayoree.objects.filter(key=extracted).values(
                    "payoree__name", "count"
                )
            )
...
def search_patterns(request):
    ...
    subcat_results = list(
        LearnedSubcat.objects.filter(key__icontains=query).values("key").distinct()
    )

    payoree_results = list(
        LearnedPayoree.objects.filter(key__icontains=query).values("key").distinct()
    )
