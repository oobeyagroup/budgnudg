...
class PayoreeReportView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        transactions = Transaction.objects.select_related("payoree").order_by(
            "-date", "description"
        )
        ...
        for txn in transactions:
            ...
            if key:
                ...
                payoree_data[key]["monthly_totals"][month_key] = payoree_data[key][
                    "monthly_totals"
                ].get(month_key, 0) + float(txn.amount)
        ...
        for payoree, data in payoree_data.items():
            if data["count"] == 1 and abs(data["total_amount"]) < 250:
                ...
                for month_key, amt in data["monthly_totals"].items():
                    # Use defensive programming to avoid KeyError
                    other_payoree["monthly_totals"][month_key] = (
                        other_payoree["monthly_totals"].get(month_key, 0) + amt
                    )
            else:
        ...
        filtered_payorees = {
            k: v for k, v in filtered_payorees.items() if v["count"] > 0
        }

        # Calculate grand totals
        grand_total_count = sum(data["count"] for data in filtered_payorees.values())
        grand_total_amount = sum(
            data["total_amount"] for data in filtered_payorees.values()
        )
        ...
        for data in filtered_payorees.values():
            for month_key, amt in data["monthly_totals"].items():
                ...
                grand_monthly_totals[month_key] = (
                    grand_monthly_totals.get(month_key, 0) + amt
                )
        ...
        if "Other Payoree" in filtered_payorees:
            sorted_payorees.append(
                ("Other Payoree", filtered_payorees["Other Payoree"])
            )
