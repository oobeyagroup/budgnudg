...
class ImportRowConverter:
    ...
    def convert_import_row_to_transaction(
        self, import_data: ImportRowData, reverse_amounts: bool = False
        ...
        try:
            ...
            transaction_data = {
                ...
                "amount": (
                    -import_data.amount if reverse_amounts else import_data.amount
                ),
                "description": import_data.description,
                "bank_account": import_data.bank_account,
                "source": import_data.source_filename
                or f"import:row:{import_data.row_index}",
            }
            ...
            # Collect any errors
            all_errors = category_result.get("errors", []) + payoree_result.get(
                "errors", []
            )
            if all_errors:
                transaction_data["categorization_error"] = all_errors[
                    0
                ]  # Use first/most significant
            ...
            logger.exception(
                "Failed to convert import row %s to transaction", import_data.row_index
            )
            return TransactionConversionResult(
                success=False,
    ...
    @trace
    def _resolve_category_from_import(
        self, import_data: ImportRowData
    ) -> Dict[str, Any]:
        """
        ...
        csv_category_name = import_data.parsed_data.get(
            "subcategory"
        ...
        if csv_category_name:
            try:
                ...
                subcategory_obj, error_code = safe_category_lookup(
                    csv_category_name, "CSV"
                )
                if error_code:
                ...
                logger.warning(
                    "CSV category lookup failed for '%s': %s", csv_category_name, e
                )
        ...
        if not subcategory_obj and not category_obj:
            ...
            if ai_category_name:
                try:
                    ...
                    logger.warning(
                        "AI category lookup failed for '%s': %s", ai_category_name, e
                    )
            else:
    ...
    @trace
    def _resolve_payoree_from_import(
        self, import_data: ImportRowData
    ) -> Dict[str, Any]:
        """
                ...
                if error_code:
                    ...
                    if "PAYOREE_LOOKUP_FAILED" in error_code:
                        ...
                        try:
                            ...
                            logger.error(
                                "Failed to create payoree %s: %s", payoree_name, e
                            )
                    else:
        ...
        return {"payoree": payoree_obj, "errors": errors}
