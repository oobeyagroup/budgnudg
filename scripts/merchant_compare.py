...
def run():
    ...
    with_pay = Transaction.objects.filter(payoree__isnull=False).select_related(
        "payoree"
    )
    ...
    # Compare extracted merchant vs assigned payoree for transactions that have a payoree
    for t in with_pay.iterator():
        extracted = norm(get_payoree_name_for_transaction(t))
    ...
    # Transactions without payoree: tally extracted merchant keys
    ...
    for t in no_pay_qs.iterator():
        extracted = norm(get_payoree_name_for_transaction(t)) or "unknown"
    ...
    for t in Transaction.objects.iterator():
        extract_counter[norm(get_payoree_name_for_transaction(t)) or "unknown"] += 1

    out_lines = []
    out_lines.append(f"Total transactions: {total}")
    out_lines.append(f"Transactions with payoree: {count_with_pay}")
    out_lines.append(
        f"Matches where extracted merchant == payoree name: {match_count} ({match_count/count_with_pay if count_with_pay else 0:.2%})"
    )
