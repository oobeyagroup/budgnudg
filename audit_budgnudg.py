...
def add(check, ok: bool, details_ok: str = "", details_fail: str = ""):
    if ok:
        ...
        results.append(
            {"check": check, "status": status_warn_or_fail(), "details": details_fail}
        )
...
candidates = {
    ...
    "mapping_service": [
        ...
        "core_ingestion.py",
        "ingest/services/mapping.py",
        "ingest/services/staging.py",
    ],
    "commit_service": [
        "transactions/services/commit.py",
        "commit_batch.py",
        "core_ingestion.py",
        "ingest/services/mapping.py",
        "ingest/services/staging.py",
                ...
                for kw in node.keywords or []:
                    if (
                        ...
                        if val in ALLOWED_STATUS:
                            ...
                            hits[val].append(
                                (str(py), getattr(node, "lineno", 0), desc, is_batchy)
                            )
                self.generic_visit(node)
...
def _report_hits(label: str, entries):
    if entries:
        ...
        details = "Found:\n    - " + "\n    - ".join(
            [fmt(e) for e in (batchy + others)]
        )
        add(f"Status usage found: '{label}'", True, details_ok=details)
    else:
        add(
            f"Status usage found: '{label}'",
            False,
...
if not ok:
    add_fail(
        "apply_profile_to_batch defined",
    ...
    results.append(
        {
            "check": "apply_profile_to_batch defined",
            "status": OK,
            "details": "Found in:\n    - " + "\n    - ".join(where),
        }
    )
...
for must in [
    ...
    add(
        f"apply_profile_to_batch uses '{must}'",
        ok,
        details_ok="Found in:\n    - " + "\n    - ".join(where),
        details_fail="Searched in:\n    - "
        + "\n    - ".join(map(str, mapping_txt.keys())),
...
ok, where = match_any(
    r"from\s+transactions\.services\.helpers\s+import\s+is_duplicate", mapping_txt
)
add(
    "Duplicate uses transactions.services.helpers.is_duplicate",
    ok,
    details_ok="Found in:\n    - " + "\n    - ".join(where),
    details_fail="Searched in:\n    - " + "\n    - ".join(map(str, mapping_txt.keys())),
)
