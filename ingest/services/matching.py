...
def _score_candidate(
    ...
    score = 0
    why: list[str] = []
    if txn.bank_account and txn.bank_account.name == bank:
        score += 3
        why.append("✓ bank")
    if check_no and f"CHECK {check_no}" in (txn.description or "").upper():
        score += 3
        why.append(f"✓ 'CHECK {check_no}'")
...
def find_candidates(
    *, bank: str, check_no: str, amount: Decimal | None, limit: int = 10
    ...
    if bank:
        qs = qs.filter(bank_account__name=bank)
    if check_no:
...
def render_description(
    csv_desc: str, *, check_no: str | None, payoree: str | None, memo: str | None
) -> str:
    parts: list[str] = []
    if csv_desc:
        parts.append(csv_desc)
    if check_no:
        parts.append(f"Check #{check_no}")
    if payoree:
        parts.append(f"Payee: {payoree}")
    if memo:
