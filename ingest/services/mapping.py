...
def map_row_with_profile(raw_row: dict, profile: FinancialAccount):
    ...
    for csv_col, model_field in mapping.items():
        ...
        if model_field == "date":
            out["date"] = raw
            try:
                out["_date"] = _parse_date(raw)
            except Exception as e:
                errors.append(f"date: {e}")
        elif model_field == "amount":
            out["amount"] = raw
            logger.debug(
                f"Processing amount: csv_col='{csv_col}', raw_value='{raw}', type={type(raw)}"
            )
            try:
...
def preview_batch(batch: ImportBatch):
    if not batch.profile:
        raise ValueError("Batch has no FinancialAccount assigned.")
    rows = []
...
def apply_profile_to_batch(
    ...
    if not isinstance(mapping, dict):
        raise TypeError(
            f"FinancialAccount.column_map must be dict; got {type(mapping)}"
        )
    ...
    for row in rows:
        try:
            ...
            logger.debug(
                "******Mapping row %s with profile %s", row.row_index, profile.name
            )
            ...
            if not suggestions.get("subcategory"):
                logger.debug(
                    "DEBUG: No multi-field subcategory found, falling back to description analysis"
                )
                ...
                combined_text = " ".join(filter(None, [desc, memo, payoree_field]))
                logger.debug(
                    "DEBUG: Combined text for analysis: '%s'", combined_text[:100]
                )
            ...
            if not suggestions.get("payoree"):
                logger.debug(
                    "DEBUG: No multi-field payoree found, falling back to description analysis"
                )
                desc = norm_description
                memo = out.get("memo", "")
                payoree_field = out.get("payoree", "")
                ...
                if payoree_sug:
                    ...
                    logger.debug(
                        "DEBUG: Generated payoree suggestion: '%s'", payoree_sug
                    )

            # Debug logging for suggestions
            logger.debug(
                "DEBUG: Row %s - Description: '%s'",
                row.row_index,
                norm_description[:50],
            )
            logger.debug(
                "DEBUG: Row %s - Subcategory suggestion: '%s'",
                row.row_index,
                suggestions.get("subcategory", "NONE"),
            )
            logger.debug(
                "DEBUG: Row %s - Payoree suggestion: '%s'",
                row.row_index,
                suggestions.get("payoree", "NONE"),
            )
            ...
            dup_payload = {
                ...
                "bank_account": bank_account_hint
                or "",  # Empty bank_account will check all accounts
            }
            is_dup = bool(is_duplicate(dup_payload))

            # Debug logging to track duplicate detection
            logger.debug(
                "Duplicate check for row %s: date=%s, amount=%s, desc='%s', bank='%s' -> is_dup=%s",
                row.row_index,
                norm_date,
                norm_amount,
                norm_description[:50],
                bank_account_hint or "EMPTY",
                is_dup,
            )
            ...
            row.save(
                update_fields=[
                    ...
                    (
                        ...
                        else "norm_description"
                    ),  # harmless re-save if no updated_at
                ]
            )
