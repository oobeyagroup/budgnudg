...
class TestIngestTemplates(TestCase):
    ...
    def test_upload_form_template_renders_correctly(self):
        ...
        assert (
            'type="file"' in response.content.decode()
            or "form-control" in response.content.decode()
        )
    ...
    def test_batch_preview_template_with_profile(self):
        ...
        response = self.client.get(
            reverse("ingest:batch_preview", kwargs={"pk": self.batch.pk})
        )
    ...
    def test_batch_preview_template_without_profile(self):
        ...
        response = self.client.get(
            reverse("ingest:batch_preview", kwargs={"pk": batch_no_profile.pk})
        )
        content = response.content.decode()
        ...
        headers_present = any(
            header in content for header in ["Date", "Description", "Amount"]
        )
    ...
    def test_profile_detail_template_shows_profile_info(self):
        ...
        response = self.client.get(
            reverse("ingest:profile_detail", kwargs={"pk": self.profile.pk})
        )
...
class TestTransactionTemplates(TestCase):
    ...
    def setUp(self):
        ...
        self.subcategory = Category.objects.create(
            name="Restaurants", parent=self.category
        )
    ...
    def test_transaction_list_template_shows_transactions(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                assert (
                    "RESTAURANT PURCHASE" in content
                    or str(self.transaction.amount) in content
                )
...
class TestTemplateErrorHandling(TestCase):
    ...
    def test_templates_handle_none_values(self):
        ...
        context = {"batch": None, "profile": None, "transactions": None}
...
class TestTemplatePerformance(TestCase):
    ...
    def test_template_handles_large_datasets(self):
        ...
        assert (
            end_time - start_time
        ) < 5.0, "Template rendering should be reasonably fast"
