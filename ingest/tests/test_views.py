...
class TestBatchPreviewView(TestCase):
    def setUp(self):
        ...
        self.row2 = ImportRow.objects.create(
            batch=self.batch, 
            row_index=1,
            raw={"Date": "2023-01-02", "Description": "Gas Station", "Amount": "-45.00"}
        )
    ...
    def test_preview_view_auto_matches_exact_profile(self):
        ...
        with patch('ingest.views.apply_profile_to_batch') as mock_apply:
            ...
            messages = list(get_messages(response.wsgi_request))
            assert any("Automatically matched and processed" in str(m) for m in messages)
    ...
    def test_preview_view_with_profile_shows_mapping_table(self):
        ...
        date_mapping = next((m for m in mapping_table if m['txn_field'] == 'date'), None)
...
class TestApplyProfileView(TestCase):
    def setUp(self):
        ...
        self.apply_url = reverse('ingest:batch_apply_profile', kwargs={'pk': self.batch.pk})
    ...
    def test_apply_profile_with_bank_account_hint(self):
        ...
        with patch('ingest.views.apply_profile_to_batch') as mock_apply:
            ...
            mock_apply.assert_called_once_with(
                self.batch,
                self.profile,
                bank_account_hint='Chase Checking'
            )
...
class TestCommitView(TestCase):
    ...
    def test_commit_view_post_with_bank_account_commits_batch(self):
        ...
        with patch('ingest.views.commit_batch') as mock_commit:
            mock_commit.return_value = ([1], [2], [])  # 1 imported, 1 duplicate, 0 skipped
...
@pytest.mark.django_db
class TestMappingProfileViews(TestCase):
    def setUp(self):
        self.client = Client()
