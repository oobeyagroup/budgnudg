...
- FinancialAccount: Column mapping configurations for different bank formats
...
class TestFinancialAccount:
    ...
    def test_profile_name_must_be_unique(self):
        ...
        FinancialAccount.objects.create(name="chase", column_map={})
    ...
    def test_profile_with_options(self):
        ...
        profile = FinancialAccount.objects.create(
            ...
            options={"date_format": "%m/%d/%Y", "amount_sign": "negative_is_expense"},
        )
    ...
    def test_profile_str_representation(self):
        ...
        profile = FinancialAccount.objects.create(
            name="test", column_map={}, description="Test Bank Account Format"
        )
    ...
    def test_profile_empty_description_fallback(self):
        ...
        profile = FinancialAccount.objects.create(name="test", column_map={})
...
class TestImportBatch:
    ...
    def test_batch_with_profile(self):
        ...
        batch = ImportBatch.objects.create(
            profile=profile, header=["Date", "Amount"], row_count=3
        )
    ...
    def test_batch_ordering(self):
        ...
        batch1 = ImportBatch.objects.create(
            source_filename="first.csv", header=[], row_count=0
        )
        batch2 = ImportBatch.objects.create(
            source_filename="second.csv", header=[], row_count=0
        )
    ...
    def test_batch_str_representation(self):
        ...
        batch = ImportBatch.objects.create(
            source_filename="my_transactions.csv", header=[], row_count=0
        )
...
class TestImportRow:
    ...
    def test_row_duplicate_flag(self):
        ...
        batch = ImportBatch.objects.create(header=[], row_count=1)
        row = ImportRow.objects.create(
            batch=batch, row_index=0, raw={"Amount": "100.00"}, is_duplicate=True
        )
    ...
    def test_row_committed_transaction_tracking(self):
        ...
        batch = ImportBatch.objects.create(header=[], row_count=1)
        row = ImportRow.objects.create(
            batch=batch, row_index=0, raw={}, committed_txn_id=12345
        )
...
class TestModelIndexes:
    ...
    def test_row_date_index(self):
        ...
        ImportRow.objects.create(
            batch=batch, row_index=0, raw={}, norm_date=date(2023, 1, 1)
        )
        ImportRow.objects.create(
            batch=batch, row_index=1, raw={}, norm_date=date(2023, 1, 2)
        )
