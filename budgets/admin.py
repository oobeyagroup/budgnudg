...
class BudgetAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Period & Amount', {
            'fields': ('year', 'month', 'amount', 'baseline_amount')
        }),
        ('AI & Integration', {
            'fields': ('is_ai_suggested', 'recurring_series')
        }),
        ('Notes & Metadata', {
    ...
    def mark_ai_suggested(self, request, queryset):
        ...
        self.message_user(
            request, 
            f"Successfully marked {updated} budget(s) as AI suggested."
        )
    ...
    def mark_user_created(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully marked {updated} budget(s) as user created."
        )
...
class BudgetPeriodAdmin(admin.ModelAdmin):
    ...
    def finalize_periods(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully finalized {updated} budget period(s)."
        )
    ...
    def unfinalize_periods(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully unfinalized {updated} budget period(s)."
        )
