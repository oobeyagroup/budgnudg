...
class BaselineCalculatorTest(TestCase):
    ...
    def create_sample_transactions(self):
        ...
        transactions = [
            ...
            {'date': date(2025, 4, 15), 'amount': 120.50, 'category': self.category, 'payoree': self.payoree, 'needs': 'Need'},
            {'date': date(2025, 5, 15), 'amount': 135.75, 'category': self.category, 'payoree': self.payoree, 'needs': 'Need'},
            {'date': date(2025, 6, 15), 'amount': 110.25, 'category': self.category, 'payoree': self.payoree, 'needs': 'Need'},
            {'date': date(2025, 7, 15), 'amount': 145.00, 'category': self.category, 'payoree': self.payoree, 'needs': 'Need'},
            {'date': date(2025, 8, 15), 'amount': 128.80, 'category': self.category, 'payoree': self.payoree, 'needs': 'Need'},
            
            # Organic foods subcategory
            {'date': date(2025, 6, 10), 'amount': 45.60, 'category': self.category, 'subcategory': self.subcategory, 'payoree': self.payoree, 'needs': 'Want'},
            {'date': date(2025, 7, 10), 'amount': 52.30, 'category': self.category, 'subcategory': self.subcategory, 'payoree': self.payoree, 'needs': 'Want'},
            {'date': date(2025, 8, 10), 'amount': 48.90, 'category': self.category, 'subcategory': self.subcategory, 'payoree': self.payoree, 'needs': 'Want'},
        ]
...
class BudgetWizardTest(TestCase):
    ...
    def test_commit_budget_draft(self, mock_transaction):
        ...
        with patch.object(self.wizard, '_create_budget_from_item') as mock_create_budget:
            mock_budget = MagicMock()
            mock_create_budget.return_value = mock_budget
            
            with patch.object(self.wizard, '_create_budget_periods') as mock_create_periods:
