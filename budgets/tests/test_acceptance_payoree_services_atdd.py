...
class TestPayoreeCentricServicesATDD(TestCase):
    ...
    def setUp(self):
        ...
        self.whole_foods = Payoree.objects.create(
            name="Whole Foods", default_category=self.groceries_category
        )
        self.electric_company = Payoree.objects.create(
            name="Electric Company", default_category=self.utilities_category
        )
        self.misc_payoree = Payoree.objects.create(
            name="Miscellaneous", default_category=self.groceries_category
        )

        # Create test account
        self.account = FinancialAccount.objects.create(
            name="Test Checking", column_map={}, description="Test account"
        )
    ...
    def test_baseline_calculator_payoree_suggestions(self):
        ...
        whole_foods_suggestion = next(
            s for s in suggestions if s["payoree_name"] == "Whole Foods"
        )
        electric_suggestion = next(
            s for s in suggestions if s["payoree_name"] == "Electric Company"
        )
    ...
    def test_baseline_calculator_payoree_aggregation(self):
        ...
        self.assertTrue(
            whole_foods_found, "Whole Foods should be in baseline calculations"
        )
    ...
    def test_budget_wizard_payoree_workflow(self):
        ...
        for suggestion in payoree_suggestions:
            if suggestion["payoree_name"] == "Whole Foods":
                ...
                self.assertEqual(
                    suggestion["category_name"], self.groceries_category.name
                )
            elif suggestion["payoree_name"] == "Electric Company":
                electric_found = True
                self.assertEqual(suggestion["payoree_name"], "Electric Company")
                self.assertEqual(
                    suggestion["category_name"], self.utilities_category.name
                )
    ...
    def test_misc_payoree_handling(self):
        ...
        self.assertIsNotNone(
            misc_suggestion, "Miscellaneous payoree should be in suggestions"
        )
