...
class TestBudgetCreationATDD(TestCase):
    ...
    def test_budget_amount_suggestions(self):
        ...
        self.assertGreater(
            len(budget_items),
            0,
            "Should have budget suggestions based on historical data",
        )
        ...
        self.assertIsNotNone(
            grocery_suggestion, "Should have suggestion for grocery store"
        )
        ...
        self.assertGreater(
            suggested_amount, Decimal("400.00")
        )  # Should suggest reasonable amount
    ...
    def test_budget_allocation_adjustment(self):
        ...
        budget_plan = BudgetPlan.objects.create(
            name="ATDD Test Budget", year=2025, month=11, is_active=True
        )
        ...
        saved_allocation = BudgetAllocation.objects.get(
            budget_plan=budget_plan, payoree=self.grocery_store
        )
    ...
    def test_budget_allocation_persistence(self):
        ...
        budget_plan = BudgetPlan.objects.create(
            name="Persistence Test Budget", year=2025, month=12, is_active=True
        )
    ...
    def test_budget_allocation_listing(self):
        ...
        plan = BudgetPlan.objects.create(
            name="Test List Budget", year=2025, month=10, is_active=True
        )

        BudgetAllocation.objects.create(
            budget_plan=plan, payoree=self.grocery_store, amount=Decimal("400.00")
        )

        BudgetAllocation.objects.create(
            budget_plan=plan, payoree=self.utility_company, amount=Decimal("90.00")
        )
    ...
    def test_transaction_budget_matching(self):
        ...
        # Given: I have active budget allocations
        plan = BudgetPlan.objects.create(
            name="Matching Test Budget", year=2025, month=10, is_active=True
        )

        allocation = BudgetAllocation.objects.create(
            budget_plan=plan, payoree=self.grocery_store, amount=Decimal("500.00")
        )
