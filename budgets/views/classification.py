...
def budget_classification_analysis(request):
    ...
    if classification_type == 'category' and category_id:
        try:
            selected_classification = Category.objects.get(id=category_id, parent__isnull=True)
            context['selected_classification'] = selected_classification
            context = _load_classification_data(request, context, 'category', selected_classification)
        except Category.DoesNotExist:
            pass
            
    elif classification_type == 'subcategory' and subcategory_id:
        try:
            selected_classification = Category.objects.get(id=subcategory_id, parent__isnull=False)
            context['selected_classification'] = selected_classification
            context = _load_classification_data(request, context, 'subcategory', selected_classification)
        except Category.DoesNotExist:
            pass
        ...
        try:
            ...
            context['selected_classification'] = selected_classification
            context = _load_classification_data(request, context, 'payoree', selected_classification)
...
def _load_classification_data(request, context, classification_type, classification_obj):
    """
    ...
    if classification_type == 'category':
        ...
        for month_info in historical_months:
            ...
            total_amount = sum(t.amount for t in month_transactions) + sum(t.amount for t in subcategory_transactions)
            historical_data[f"{month_info['year']}-{month_info['month']:02d}"] = abs(total_amount)
    ...
    if active_budget_plan:
        ...
        if allocation:
            for month_info in budget_months:
                budget_data[f"{month_info['year']}-{month_info['month']:02d}"] = abs(allocation.amount)
